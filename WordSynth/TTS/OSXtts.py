...
class TTSOSXtts(object):
    def __init__(self):
        ...
        self._consonant_vowel_map_data_file = open(
            "share/consonant_vowel_map_osxtts.json", "r")
        self.consonant_vowel_map = json.loads(
            self._consonant_vowel_map_data_file.read())
    ...
    def phonetize(self, word):
        ...
        for match in matches:
            if match[0]:
                characters.append(
                    {"type": "PUNCTUATION_DATA", "symbol": match[0]})
            if match[1]:
                characters.append(
                    {"type": "PROSODIC_CONTROL", "symbol": match[1]})
            if match[2]:
                characters.append({
                    "type": "PHONEME",
                    "symbol": match[2],
